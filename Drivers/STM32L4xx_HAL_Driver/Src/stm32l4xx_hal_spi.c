...
HAL_StatusTypeDef HAL_SPI_Init(SPI_HandleTypeDef* hspi)
{
  ...
  WRITE_REG(hspi->Instance->CR1,
            ((hspi->Init.Mode & (SPI_CR1_MSTR | SPI_CR1_SSI)) |
             (hspi->Init.Direction & (SPI_CR1_RXONLY | SPI_CR1_BIDIMODE)) | (hspi->Init.CLKPolarity & SPI_CR1_CPOL) |
             (hspi->Init.CLKPhase & SPI_CR1_CPHA) | (hspi->Init.NSS & SPI_CR1_SSM) |
             (hspi->Init.BaudRatePrescaler & SPI_CR1_BR_Msk) | (hspi->Init.FirstBit & SPI_CR1_LSBFIRST) |
  ...
  WRITE_REG(hspi->Instance->CR2,
            (((hspi->Init.NSS >> 16U) & SPI_CR2_SSOE) | (hspi->Init.TIMode & SPI_CR2_FRF) |
             (hspi->Init.NSSPMode & SPI_CR2_NSSP) | (hspi->Init.DataSize & SPI_CR2_DS_Msk) | (frxth & SPI_CR2_FRXTH)));
...
HAL_StatusTypeDef HAL_SPI_DMAStop(SPI_HandleTypeDef* hspi)
{
  ...
  /* The Lock is not implemented on this API to allow the user application
     ...
     HAL_SPI_TxRxCpltCallback(): when calling HAL_DMA_Abort() API the DMA TX/RX Transfer complete interrupt is generated
...
static void SPI_DMATransmitReceiveCplt(DMA_HandleTypeDef* hdma)
{
  ...
  if ((hdma->Instance->CCR & DMA_CCR_CIRC) != DMA_CCR_CIRC)
  {
    ...
    if (hspi->Init.CRCCalculation == SPI_CRCCALCULATION_ENABLE)
    {
      ...
      {
        if (SPI_WaitFifoStateUntilTimeout(hspi, SPI_FLAG_FRLVL, SPI_FRLVL_HALF_FULL, SPI_DEFAULT_TIMEOUT, tickstart) !=
            HAL_OK)
        {
          /* Error on the CRC reception */
          SET_BIT(hspi->ErrorCode, HAL_SPI_ERROR_CRC);
