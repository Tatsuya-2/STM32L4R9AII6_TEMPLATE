  ...
  ******************************************************************************
  ...
  [..]
    ...
    (#)Initialize the SDMMC low level resources by implementing the HAL_SD_MspInit() API:
        ...
        (##) On STM32L4Rx/STM32L4Sxx devices, no DMA configuration is need, an internal DMA for SDMMC Peripheral is
  used.
                                     ...
                                     uint32_t Timeout)
{
  ...
  if (hsd->State == HAL_SD_STATE_READY)
  {
    ...
    dataremaining = config.DataLength;
    while (
        !__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_TXUNDERR | SDMMC_FLAG_DCRCFAIL | SDMMC_FLAG_DTIMEOUT | SDMMC_FLAG_DATAEND))
    {
                                       ...
                                       uint32_t NumberOfBlocks)
{
  ...
  if (hsd->State == HAL_SD_STATE_READY)
  {
    ...
    __HAL_SD_ENABLE_IT(
        hsd, (SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT | SDMMC_IT_RXOVERR | SDMMC_IT_DATAEND | SDMMC_FLAG_RXFIFOHF));
                                        ...
                                        uint32_t NumberOfBlocks)
{
  ...
  if (hsd->State == HAL_SD_STATE_READY)
  {
    ...
    __HAL_SD_ENABLE_IT(
        hsd, (SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT | SDMMC_IT_TXUNDERR | SDMMC_IT_DATAEND | SDMMC_FLAG_TXFIFOHE));
...
void HAL_SD_IRQHandler(SD_HandleTypeDef* hsd)
{
  ...
  else if (__HAL_SD_GET_FLAG(hsd, SDMMC_FLAG_DATAEND) != RESET)
  {
    ...
    __HAL_SD_DISABLE_IT(hsd, SDMMC_IT_DATAEND | SDMMC_IT_DCRCFAIL | SDMMC_IT_DTIMEOUT | SDMMC_IT_TXUNDERR |
