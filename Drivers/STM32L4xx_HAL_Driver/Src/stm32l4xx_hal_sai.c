  ...
  [..]
    (@) Make sure that either:
        ...
        (+@) External clock source is configured after setting correctly
             the define constant EXTERNAL_SAI1_CLOCK_VALUE or EXTERNAL_SAI2_CLOCK_VALUE in the stm32l4xx_hal_conf.h
  file.
...
void HAL_SAI_IRQHandler(SAI_HandleTypeDef* hsai)
{
  if (hsai->State != HAL_SAI_STATE_RESET)
  {
    ...
    else if (((itflags & SAI_FLAG_OVRUDR) == SAI_FLAG_OVRUDR) && ((itsources & SAI_IT_OVRUDR) == SAI_IT_OVRUDR))
    {
      ...
      /* the transfer is not stopped, we will forward the information to the user and we let the user decide what needs
       * to be done */
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1)
