...
typedef struct
{
  ...
  uint32_t Mckdiv; /*!< Specifies the master clock divider.
                        This parameter must be a number between Min_Data = 0 and Max_Data = 63 on STM32L4Rx/STM32L4Sx
                      devices. This parameter must be a number between Min_Data = 0 and Max_Data = 15 on other devices.
...
#define IS_SAI_SUPPORTED_PROTOCOL(PROTOCOL)                                                                            \
  (((PROTOCOL) == SAI_I2S_STANDARD) || ((PROTOCOL) == SAI_I2S_MSBJUSTIFIED) || ((PROTOCOL) == SAI_I2S_LSBJUSTIFIED) || \
   ((PROTOCOL) == SAI_PCM_LONG) || ((PROTOCOL) == SAI_PCM_SHORT))
...
#define IS_SAI_BLOCK_SYNCHRO(SYNCHRO)                                                                                  \
  (((SYNCHRO) == SAI_ASYNCHRONOUS) || ((SYNCHRO) == SAI_SYNCHRONOUS) || ((SYNCHRO) == SAI_SYNCHRONOUS_EXT_SAI1) ||     \
...
#define IS_SAI_BLOCK_COMPANDING_MODE(MODE)                                                                             \
  (((MODE) == SAI_NOCOMPANDING) || ((MODE) == SAI_ULAW_1CPL_COMPANDING) || ((MODE) == SAI_ALAW_1CPL_COMPANDING) ||     \
   ((MODE) == SAI_ULAW_2CPL_COMPANDING) || ((MODE) == SAI_ALAW_2CPL_COMPANDING))
