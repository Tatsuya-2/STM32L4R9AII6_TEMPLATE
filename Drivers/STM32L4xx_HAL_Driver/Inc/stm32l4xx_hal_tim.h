...
typedef struct
{
  ...
  uint32_t BreakFilter; /*!< Specifies the break input filter.This parameter can be a number between Min_Data = 0x0 and
                           Max_Data = 0xF */
  ...
  uint32_t Break2Filter; /*!< TIM break2 input filter.This parameter can be a number between Min_Data = 0x0 and Max_Data
                            = 0xF */
...
#endif /* USE_HAL_TIM_REGISTER_CALLBACKS */
{
  ...
  void (*PeriodElapsedHalfCpltCallback)(struct __TIM_HandleTypeDef* htim); /*!< TIM Period Elapsed half complete
                                                                              Callback               */
  ...
  void (*PWM_PulseFinishedHalfCpltCallback)(struct __TIM_HandleTypeDef* htim); /*!< TIM PWM Pulse Finished half complete
                                                                                  Callback           */
...
#define TIM_ENCODERMODE_TI12                                                                                           \
  (TIM_SMCR_SMS_1 | TIM_SMCR_SMS_0) /*!< Quadrature encoder mode 3, x4 mode, counts up/down on both TI1FP1 and TI2FP2  \
...
#define TIM_COMMUTATION_TRGI                                                                                           \
  TIM_CR2_CCUS /*!< When Capture/compare control bits are preloaded, they are updated by setting the COMG bit or when  \
                  an rising edge occurs on trigger input */
...
#define TIM_AUTOMATICOUTPUT_ENABLE                                                                                     \
  TIM_BDTR_AOE /*!< MOE can be set by software or automatically at the next update event (if none of the break inputs  \
                  BRK and BRK2 is active) */
/**
 * @}
 */
...
#define TIM_TRGO2_OC4REF_RISING_OC6REF_FALLING                                                                         \
  (TIM_CR2_MMS2_3 | TIM_CR2_MMS2_2 | TIM_CR2_MMS2_0) /*!< OC4REF rising or OC6REF falling edges generate pulses on     \
                                                        TRGO2 */
...
#define TIM_BREAK_SYSTEM_PVD                                                                                           \
  SYSCFG_CFGR2_PVDL /*!< Enables and locks the PVD connection with TIM1/8/15/16/17 Break Input and also the PVDE and   \
...
#define IS_TIM_CLEARINPUT_SOURCE(__MODE__)                                                                             \
  (((__MODE__) == TIM_CLEARINPUTSOURCE_ETR) || ((__MODE__) == TIM_CLEARINPUTSOURCE_OCREFCLR) ||                        \
...
#define IS_TIM_DMA_BASE(__BASE__)                                                                                      \
  (((__BASE__) == TIM_DMABASE_CR1) || ((__BASE__) == TIM_DMABASE_CR2) || ((__BASE__) == TIM_DMABASE_SMCR) ||           \
   ...
   ((__BASE__) == TIM_DMABASE_CCR6) || ((__BASE__) == TIM_DMABASE_OR2) || ((__BASE__) == TIM_DMABASE_OR3))
...
#define IS_TIM_PWM_MODE(__MODE__)                                                                                      \
  (((__MODE__) == TIM_OCMODE_PWM1) || ((__MODE__) == TIM_OCMODE_PWM2) || ((__MODE__) == TIM_OCMODE_COMBINED_PWM1) ||   \
   ((__MODE__) == TIM_OCMODE_COMBINED_PWM2) || ((__MODE__) == TIM_OCMODE_ASSYMETRIC_PWM1) ||                           \
...
#define IS_TIM_BREAK_SYSTEM(__CONFIG__)                                                                                \
  (((__CONFIG__) == TIM_BREAK_SYSTEM_ECC) || ((__CONFIG__) == TIM_BREAK_SYSTEM_PVD) ||                                 \
   ((__CONFIG__) == TIM_BREAK_SYSTEM_SRAM2_PARITY_ERROR) || ((__CONFIG__) == TIM_BREAK_SYSTEM_LOCKUP))
