...
typedef struct
{
  ...
  uint32_t NbPages;   /*!< Number of pages to be erased.
                           This parameter must be a value between 1 and (max number of pages in the bank - value of
                         initial page)*/
...
typedef struct
{
  ...
  uint32_t WRPEndOffset;   /*!< Write protection end offset (used for OPTIONBYTE_WRP).
                                This parameter must be a value between WRPStartOffset and (max number of pages in the bank
                              - 1) */
...
typedef struct
{
  ...
  __IO FLASH_ProcedureTypeDef ProcedureOnGoing; /* Internal variable to indicate which procedure is ongoing or not in IT
                                                   context */
...
#define OB_IWDG_STDBY_RUN                                                                                              \
  ((uint32_t)FLASH_OPTR_IWDG_STDBY) /*!< Independent watchdog counter is running in Standby mode */
/**
 * @}
 */
...
#define OB_PCROP_RDP_ERASE                                                                                             \
  ((uint32_t)FLASH_PCROP1ER_PCROP_RDP) /*!< PCROP area is erased when the RDP level is                                 \
...
#define IS_FLASH_PAGE(PAGE)                                                                                            \
  (((((*((uint16_t*)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x200U) ?                                               \
        ((PAGE) < 256U) :                                                                                              \
        ((((*((uint16_t*)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x100U) ? ((PAGE) < 128U) : ((PAGE) < 256U))))
#else
#define IS_FLASH_PAGE(PAGE)                                                                                            \
  (((((*((uint16_t*)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x100U) ?                                               \
        ((PAGE) < 128U) :                                                                                              \
        ((((*((uint16_t*)FLASH_SIZE_DATA_REGISTER)) & (0x0FFFU)) == 0x80U) ? ((PAGE) < 64U) : ((PAGE) < 128U))))
#endif
