...
#define IS_FMC_NORSRAM_BANK(__BANK__)                                                                                  \
  (((__BANK__) == FMC_NORSRAM_BANK1) || ((__BANK__) == FMC_NORSRAM_BANK2) || ((__BANK__) == FMC_NORSRAM_BANK3) ||      \
   ((__BANK__) == FMC_NORSRAM_BANK4))
#define IS_FMC_MUX(__MUX__) (((__MUX__) == FMC_DATA_ADDRESS_MUX_DISABLE) || ((__MUX__) == FMC_DATA_ADDRESS_MUX_ENABLE))
...
#define IS_FMC_NORSRAM_MEMORY_WIDTH(__WIDTH__)                                                                         \
  (((__WIDTH__) == FMC_NORSRAM_MEM_BUS_WIDTH_8) || ((__WIDTH__) == FMC_NORSRAM_MEM_BUS_WIDTH_16) ||                    \
   ((__WIDTH__) == FMC_NORSRAM_MEM_BUS_WIDTH_32))
#define IS_FMC_PAGESIZE(__SIZE__)                                                                                      \
  (((__SIZE__) == FMC_PAGE_SIZE_NONE) || ((__SIZE__) == FMC_PAGE_SIZE_128) || ((__SIZE__) == FMC_PAGE_SIZE_256) ||     \
   ((__SIZE__) == FMC_PAGE_SIZE_512) || ((__SIZE__) == FMC_PAGE_SIZE_1024))
#if defined(FMC_BCR1_WFDIS)
#define IS_FMC_WRITE_FIFO(__FIFO__) (((__FIFO__) == FMC_WRITE_FIFO_DISABLE) || ((__FIFO__) == FMC_WRITE_FIFO_ENABLE))
#endif /* FMC_BCR1_WFDIS */
#define IS_FMC_ACCESS_MODE(__MODE__)                                                                                   \
  (((__MODE__) == FMC_ACCESS_MODE_A) || ((__MODE__) == FMC_ACCESS_MODE_B) || ((__MODE__) == FMC_ACCESS_MODE_C) ||      \
   ((__MODE__) == FMC_ACCESS_MODE_D))
#if defined(FMC_BCRx_NBLSET)
#define IS_FMC_NBL_SETUPTIME(__NBL__)                                                                                  \
  (((__NBL__) == FMC_NBL_SETUPTIME_0) || ((__NBL__) == FMC_NBL_SETUPTIME_1) || ((__NBL__) == FMC_NBL_SETUPTIME_2) ||   \
   ((__NBL__) == FMC_NBL_SETUPTIME_3))
#endif /* FMC_BCRx_NBLSET */
#define IS_FMC_BURSTMODE(__STATE__)                                                                                    \
  (((__STATE__) == FMC_BURST_ACCESS_MODE_DISABLE) || ((__STATE__) == FMC_BURST_ACCESS_MODE_ENABLE))
#define IS_FMC_WAIT_POLARITY(__POLARITY__)                                                                             \
  (((__POLARITY__) == FMC_WAIT_SIGNAL_POLARITY_LOW) || ((__POLARITY__) == FMC_WAIT_SIGNAL_POLARITY_HIGH))
...
#define IS_FMC_WRITE_OPERATION(__OPERATION__)                                                                          \
  (((__OPERATION__) == FMC_WRITE_OPERATION_DISABLE) || ((__OPERATION__) == FMC_WRITE_OPERATION_ENABLE))
...
#define IS_FMC_ASYNWAIT(__STATE__)                                                                                     \
  (((__STATE__) == FMC_ASYNCHRONOUS_WAIT_DISABLE) || ((__STATE__) == FMC_ASYNCHRONOUS_WAIT_ENABLE))
...
#define IS_FMC_CONTINOUS_CLOCK(__CCLOCK__)                                                                             \
  (((__CCLOCK__) == FMC_CONTINUOUS_CLOCK_SYNC_ONLY) || ((__CCLOCK__) == FMC_CONTINUOUS_CLOCK_SYNC_ASYNC))
...
#define IS_FMC_WAIT_FEATURE(__FEATURE__)                                                                               \
  (((__FEATURE__) == FMC_NAND_WAIT_FEATURE_DISABLE) || ((__FEATURE__) == FMC_NAND_WAIT_FEATURE_ENABLE))
#define IS_FMC_NAND_MEMORY_WIDTH(__WIDTH__)                                                                            \
  (((__WIDTH__) == FMC_NAND_MEM_BUS_WIDTH_8) || ((__WIDTH__) == FMC_NAND_MEM_BUS_WIDTH_16))
#define IS_FMC_ECC_STATE(__STATE__) (((__STATE__) == FMC_NAND_ECC_DISABLE) || ((__STATE__) == FMC_NAND_ECC_ENABLE))

#define IS_FMC_ECCPAGE_SIZE(__SIZE__)                                                                                  \
  (((__SIZE__) == FMC_NAND_ECC_PAGE_SIZE_256BYTE) || ((__SIZE__) == FMC_NAND_ECC_PAGE_SIZE_512BYTE) ||                 \
   ((__SIZE__) == FMC_NAND_ECC_PAGE_SIZE_1024BYTE) || ((__SIZE__) == FMC_NAND_ECC_PAGE_SIZE_2048BYTE) ||               \
   ((__SIZE__) == FMC_NAND_ECC_PAGE_SIZE_4096BYTE) || ((__SIZE__) == FMC_NAND_ECC_PAGE_SIZE_8192BYTE))
