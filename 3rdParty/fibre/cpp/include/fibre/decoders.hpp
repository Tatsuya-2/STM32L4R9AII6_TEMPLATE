...
public:
  ...
  explicit StreamDecoder_from_BlockDecoder(Args&&... args) : block_decoder_(std::forward<Args>(args)...)
  {
...
public:
  ...
  BlockDecoder_from_ByteDecoder(Args&&... args) : byte_decoder_(std::forward<Args>(args)...)
  {
...
public:
  ...
  StreamDecoder_from_ByteDecoder(Args&&... args) : byte_decoder_(std::forward<Args>(args)...)
  {
    EXPECT_TYPE(T, ByteDecoder);
  }
...
public:
  ...
  VarintByteDecoder(T& state_variable) : state_variable_(state_variable)
  {
  }
...
{
public:
  CRC8BlockDecoder(TDecoder&& inner_decoder) : inner_decoder_(std::forward<TDecoder>(inner_decoder))
  {
  }
