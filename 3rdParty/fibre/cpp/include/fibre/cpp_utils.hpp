...
Now you want to create a wrapper class. This wrapper class should provide the exact same constructors as `InnerClass`,
so you use perfect forwarding:
```
class WrapperClass {
public:
...
public:
  ...
  {
    return std::is_base_of<U, DecayedT>::value && TypeChecker<Ts...>::template all_are<U>();
  }
};
...
#define ENABLE_IF(...) typename = std::enable_if_t<__VA_ARGS__>

#define ENABLE_IF_SAME(a, b, type)                                                                                     \
  template <typename T = a>                                                                                            \
  typename std::enable_if_t<std::is_same<T, b>::value, type>
...
public:
  ...
  static TRet invoke(TObj& obj, TRet (TObj::*func_ptr)(TArgs...), std::tuple<TArgs...> packed_args,
                     TUnpackedArgs... args)
  {
