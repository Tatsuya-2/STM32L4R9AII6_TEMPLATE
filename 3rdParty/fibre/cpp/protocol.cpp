...
int StreamBasedPacketSink::process_packet(const uint8_t* buffer, size_t length)
{
  ...
  uint8_t header[] = { CANONICAL_PREFIX, static_cast<uint8_t>(length), 0 };
  ...
  uint8_t crc16_buffer[] = { (uint8_t)((crc16 >> 8) & 0xff), (uint8_t)((crc16 >> 0) & 0xff) };
...
{
  return (endpoint_ref.json_crc == json_crc_) && (endpoint_ref.endpoint_id < n_endpoints_);
}
